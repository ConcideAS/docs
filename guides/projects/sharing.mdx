...
If your organization uses multiple email domains, your administrator can link them so that users across all domains can find and share projects with each other. When this is enabled, searching for a colleague in the share dialog returns results from every linked domain — not just your own.

This is configured at the instance level and requires no action from individual users. If you expect to see colleagues from another domain but don't, contact your administrator to verify that both domains are linked.
