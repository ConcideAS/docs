...
To clear every memory in the project, select **Delete all**, then confirm. **Delete all** is hidden when the list is empty, when a search or tag filter is applied, and for members with view-only access.
