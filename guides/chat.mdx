...
When you switch between conversations, Saga automatically saves your unsent work in each chat. This includes your message text, attached files, selected assistant, chosen sources, and any other input-bar settings. When you return to a chat, everything is restored exactly as you left it — no need to retype or re-attach anything.

Drafts are kept in memory for the duration of your session. They are cleared when you send the message or manually reset the input.

## Starting a new chat with your current files

When you click **New Chat** from inside an existing conversation, Saga carries your uploaded files into the new chat automatically. This is useful when you want to ask a different question about the same documents without re-uploading them.

The files are copied in the background, so there may be a brief moment while they become ready in the new conversation.
