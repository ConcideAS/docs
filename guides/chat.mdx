...
You can also start a chat from inside a project. When you do, the AI automatically has access to that project's knowledge base — it can see every active document and search or fetch them on demand without you needing to attach anything.

## How context works

The AI's answers depend on what information it has access to. In a **project chat**, the AI can search and retrieve documents from the project's knowledge base on demand. In a **global chat** (outside any project), you provide context by attaching files, selecting organizational sources, or enabling web search. You can also combine multiple sources in a single conversation.
