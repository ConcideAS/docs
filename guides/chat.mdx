...
The chat header gives you quick access to common actions without leaving the conversation.

Click the chat title to open a dropdown with the following options:

- **Favorite** — star or unstar the chat for quick access in your history
- **Rename** — edit the title inline, right from the header
- **Change project** — move a project chat to a different project
- **Remove from project** — detach the chat from its current project
- **Delete** — permanently delete the conversation

If the chat isn't linked to a project, the header shows an **Add to project** option in the breadcrumb. Click it to pick a project and move the chat there. You can undo the move from the notification that appears.

For project chats, the breadcrumb shows the project name first, then the chat title. Click the project name to navigate to the project.

## Export a conversation

To download an entire chat as a Word document, click **Export chat** in the chat header. The export includes all messages, citations, and the conversation title. You can also export individual messages from the message action menu.
...
Your conversations are saved in the chat history, accessible from the sidebar. You can search chats by title, star favorites for quick access, rename conversations, or delete them. You can also rename or favorite a chat directly from the [chat header](#chat-header-actions) while you're in the conversation. Chat history is organized with favorites first, then by most recent activity.
