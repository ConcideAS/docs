...
| **Deep Web Research** | Runs a thorough multi-source web research pass and returns a cited report (see [Deep web research](/guides/chat/deep-research)) |
...
You can run workflows directly from a chat without opening the builder. Ask the AI to handle a multi-step task and it looks through your library for a matching workflow and runs it inline — pausing for input the same way it would in the builder, and showing each step in the chat thread as it goes.

If nothing in your library fits, the AI can sketch a new workflow on the fly. It proposes a draft — a sequence of steps designed for what you asked — and runs it. After the run, the draft lands in your library under **Personal**, where you can keep it, edit it in the builder, or discard it.

This means you can ask for something like "translate these contracts to English, summarize each, and flag any unusual clauses" and let Saga build and run the right flow without you laying out the canvas yourself.
