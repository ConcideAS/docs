...
When the results come back, you can attach individual documents to the conversation using the **Add to chat** button on each result. This gives the AI access to the full document text — not just the snippet from the search — so it can draw on the complete source in later messages.
