...
When auto drafting is enabled, the AI can choose to create documents in the side panel to keep the chat clean. For example, it might produce a memo, a summary, a contract clause, or any other written output as a standalone document rather than inline in the conversation. These appear as downloadable files you can open in the side panel or export as Word documents.

Toggle **Auto drafting** on or off from the toolbar. When it's off, all AI output appears directly in the chat. When it's on, the AI decides whether the request calls for a separate document and generates one when appropriate.
