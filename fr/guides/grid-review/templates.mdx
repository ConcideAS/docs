...
## Publier sur Saga Lab

Si votre compte dispose des autorisations de gestion Saga Lab, vous pouvez enregistrer des modèles directement sur Saga Lab lors de la création. Lors de la création d'un nouveau modèle, utilisez le menu déroulant **Save to** pour choisir entre **Personal** et **Saga Lab**. En sélectionnant **Saga Lab**, le modèle est publié dans la collection curatée de Saga Lab, le rendant disponible pour toutes les organisations.

<Note>
L'option **Save to** n'apparaît que lors de la création d'un modèle et n'est disponible que pour les utilisateurs disposant d'un accès de gestion Saga Lab. Si vous modifiez un modèle existant, sa destination ne peut pas être changée.
</Note>
