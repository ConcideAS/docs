...
Vous pouvez ajouter des modèles en favoris pour un accès rapide et filtrer par [domaine de pratique](/fr/guides/quickstart#domaines-de-pratique) ou catégorie lorsque la bibliothèque devient volumineuse.

## Enregistrer vos propres modèles

Après avoir construit une grille avec des questions qui fonctionnent bien, cliquez sur **Enregistrer comme modèle** dans l'en-tête de la grille. Donnez-lui un nom et une description, et étiquetez-le avec un ou plusieurs domaines de pratique pour le retrouver plus facilement par la suite. Il est ajouté à vos modèles personnels. Si c'est quelque chose dont toute votre équipe pourrait bénéficier, vous pouvez demander à le rendre disponible à l'échelle de l'organisation — un administrateur l'examine et l'approuve.
