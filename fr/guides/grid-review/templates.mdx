...
Si vous disposez des autorisations de gestion Saga Lab, la boîte de dialogue **Enregistrer comme modèle** inclut un sélecteur de destination où vous pouvez choisir entre **Personnel** et **Saga Lab**. En sélectionnant **Saga Lab**, le modèle est publié directement dans la bibliothèque Saga Lab, le rendant disponible pour tous les utilisateurs de Saga. L'option de demander une disponibilité à l'échelle de l'organisation n'est pas affichée lors de la publication sur Saga Lab, car les modèles Saga Lab sont partagés entre toutes les organisations.
