...
| **Deep Web Research** | Lance une recherche web multi-sources approfondie et renvoie un rapport sourcé (voir [Deep web research](/fr/guides/chat/deep-research)) |
...
## Workflows dans le chat

Vous pouvez exécuter des workflows directement depuis un chat sans ouvrir le constructeur. Demandez à l'IA de gérer une tâche en plusieurs étapes : elle parcourt votre bibliothèque à la recherche d'un workflow correspondant et l'exécute en ligne — en mettant en pause pour les entrées de la même manière que dans le constructeur, et en affichant chaque étape dans le fil du chat à mesure de la progression.

Si rien dans votre bibliothèque ne convient, l'IA peut esquisser un nouveau workflow à la volée. Elle propose un brouillon — une séquence d'étapes conçue pour ce que vous avez demandé — et l'exécute. Après l'exécution, le brouillon arrive dans votre bibliothèque sous **Personnel**, où vous pouvez le conserver, le modifier dans le constructeur ou l'écarter.

Cela signifie que vous pouvez demander quelque chose comme « traduis ces contrats en anglais, résume chacun et signale les clauses inhabituelles » et laisser Saga construire et exécuter le bon flux sans que vous disposiez vous-même le canevas.
