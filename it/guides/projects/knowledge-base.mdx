...
Puoi visualizzare in anteprima determinati tipi di file direttamente nel pannello laterale senza doverli scaricare. Clicca su un documento nella knowledge base o su una citazione nella chat per aprirlo.

| Tipo di file    | Comportamento dell'anteprima                                                       |
| --------------- | ---------------------------------------------------------------------------------- |
| Immagini        | I file PNG, JPG, JPEG, GIF, WEBP e TIFF si aprono in un visualizzatore di immagini con controlli di zoom. Clicca il pulsante di ripristino per tornare alla dimensione originale. |
| CSV             | I file CSV vengono visualizzati in una tabella a foglio di calcolo con colonne etichettate e righe numerate. Puoi ingrandire e ridurre per regolare la visualizzazione. |
| Testo semplice  | I file TXT vengono mostrati con la loro formattazione originale preservata.          |
| PDF e DOCX      | Questi continuano ad aprirsi nel visualizzatore di documenti integrato con navigazione tra le pagine ed evidenziazione delle citazioni. |

Per i file CSV e di testo semplice di grandi dimensioni, viene mostrata solo una parte del contenuto. Un banner in basso ti informa che l'anteprima è troncata — scarica il file per vedere il contenuto completo.
