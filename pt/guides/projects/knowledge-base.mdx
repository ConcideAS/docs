...
## Pré-visualização de ficheiros

Pode pré-visualizar determinados tipos de ficheiro diretamente no painel lateral sem os descarregar. Clique num documento na base de conhecimento ou numa citação no chat para o abrir.

| Tipo de ficheiro | Comportamento da pré-visualização                                                  |
| ---------------- | ---------------------------------------------------------------------------------- |
| Imagens          | Os ficheiros PNG, JPG, JPEG, GIF, WEBP e TIFF abrem num visualizador de imagens com controlos de zoom. Clique no botão de repor para voltar ao tamanho original. |
| CSV              | Os ficheiros CSV são apresentados numa tabela tipo folha de cálculo com colunas etiquetadas e linhas numeradas. Pode aproximar e afastar o zoom para ajustar a vista. |
| Texto simples    | Os ficheiros TXT são apresentados com a sua formatação original preservada.          |
| PDF e DOCX       | Estes continuam a abrir no visualizador de documentos integrado com navegação por páginas e destaque de citações. |

Para ficheiros CSV e de texto simples grandes, apenas uma parte do conteúdo é mostrada. Um banner na parte inferior informa que a pré-visualização está truncada — descarregue o ficheiro para ver o conteúdo completo.
