...
Todas as pessoas no espaço de trabalho Saga têm um destes papéis:
...
**Billing Admin** — Gere a subscrição em **Faturação** no painel de administração. No Solo, quem compra a subscrição recebe este papel automaticamente e pode atualizar métodos de pagamento, transferir faturas e convidar ou remover membros da equipa a partir daí. O papel também pode ser atribuído noutros espaços de trabalho, mas só abre Faturação, não Utilizadores, definições, integrações nem bibliotecas. Consulte [Solo](/pt/guides/administration/solo).

Se a sua subscrição puder adicionar cobertura de jurisprudência Moonlit, um Billing Admin pode comprá-la em **Painel de administração** → **Faturação**. Os lugares Moonlit permanecem correspondentes à sua contagem de lugares Saga quando adiciona ou remove lugares. A remoção e outras regras de faturação seguem os termos do portal de faturação apresentados na compra.
...
- **Formato de data** — como as datas aparecem no espaço de trabalho (`dd-mm-yyyy`, `mm-dd-yyyy` ou `yyyy-mm-dd`)
...
- **[NetDocuments](/pt/guides/integrations/netdocuments)** — ative com a chave de acesso do fornecedor e a região do escritório
- **[Dillien](/pt/guides/integrations/dillien)** — ative para importar documentos das data rooms Dillien
- **[Ferramentas MCP](/pt/guides/integrations/mcp-server/connect-tools)** — ligue ferramentas externas que a IA pode usar no chat
...
Os administradores podem configurar o aspeto das exportações Word geradas pela Saga. Use [Estilo dos documentos](/pt/guides/administration/document-styling) para definir fontes, títulos, margens, cabeçalhos, rodapés, números de página e logótipos para toda a sociedade.

## Política de IA

Os administradores podem carregar um documento de **Política de IA** por idioma. A Saga mostra essa política como o aviso no chat. Se um idioma não tiver ficheiro próprio, a Saga usa a política do idioma predefinido do espaço de trabalho.

## Retenção de dados

Os administradores definem durante quanto tempo a Saga guarda o histórico de chat, os chats de projeto e as revisões em grelha. Abra **Retenção de dados**, escolha um prazo e reveja a **Pré-visualização do efeito** antes de guardar. A pré-visualização mostra quantos chats e revisões serão eliminados por utilizador.

As alterações entram em vigor na data indicada no painel. Os itens mais antigos do que o prazo escolhido são eliminados após um período de tolerância de 24 horas. Selecione **Ilimitado** se o espaço de trabalho não deve eliminar chats ou revisões automaticamente.

A retenção de dados não elimina outros dados do projeto, como documentos.
